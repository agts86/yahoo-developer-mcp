...
export class YahooApiKeyGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    this.logger.debug(
      `Auth attempt - Authorization: ${authHeader ? '[PRESENT]' : '[MISSING]'}`,
    );

    try {
      // ヘッダーからYahoo API Keyを抽出
      const yahooAppId = this.configService.extractYahooApiKey(
      ...
      if (!yahooAppId || yahooAppId.trim() === '') {
        ...
        throw new UnauthorizedException(
          'Yahoo API Key is required. Provide via Authorization header (Bearer token).',
        );
      }

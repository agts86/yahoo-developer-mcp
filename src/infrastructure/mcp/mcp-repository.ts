...
import {
  ...
  GeocodeQuery,
  ReverseGeocodeQuery,
} from '../../domain/mcp/queries/yahooQueries.js';

const BASE_LOCAL_SEARCH =
  'https://map.yahooapis.jp/search/local/V1/localSearch';
const BASE_GEOCODE = 'https://map.yahooapis.jp/geocode/V1/geoCoder';
const BASE_REVERSE_GEOCODE =
  'https://map.yahooapis.jp/geocode/V1/reverseGeoCoder';
...
export class McpRepository implements IMcpRepository {
  ...
  async geocode(query: GeocodeQuery): Promise<GeocodeResult> {
    ...
    const raw = await this.httpClient.get<GeocodeResponseRaw, GeocodeQuery>(
      BASE_GEOCODE,
      { query },
    );
  ...
  async reverseGeocode(
    ...
    this.logger.debug(
      `Reverse Geocode Request params: ${JSON.stringify(query)}`,
    );
    const raw = await this.httpClient.get<
      ReverseGeocodeResponseRaw,
      ReverseGeocodeQuery
    >(BASE_REVERSE_GEOCODE, { query });

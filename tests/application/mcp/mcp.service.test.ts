...
describe('McpService', () => {
  ...
  test('未知メソッドは MethodNotFoundError を投げる', async () => {
    await expect(
      service.handleHttpMcpMessage({ id: 'x', method: 'unknown' }),
  ...
  test('handleHttpMcpError が MethodNotFound を 400 に変換する', () => {
    const error = Object.assign(new Error('Method not found: foo'), {
      name: 'MethodNotFoundError',
      id: '123',
    });

    expect(() => service.handleHttpMcpError(error, '123')).toThrow(
      HttpException,
    );
    try {
  ...
  test('executeToolByName でツールを実行する', async () => {
    ...
    const result = await service.executeToolByName(
      'geocode',
      { q: 'tokyo' },
      'appid',
    );
  ...
  test('executeToolByName で存在しないツールなら UnknownToolError', async () => {
    await expect(
      service.executeToolByName('not-found', {}, 'appid'),
    ).rejects.toMatchObject({
      name: 'UnknownToolError',
      message: 'Unknown tool: not-found',
    });
  });
  ...
  test('handleStreamableHttpRequest で必須ヘッダーを補完し処理を委譲する', async () => {
    ...
    jest
      .spyOn(service as any, 'createStreamableContext')
      .mockReturnValue({ server, transport });
    const runSpy = jest
      .spyOn(service as any, 'runStreamableHandling')
      .mockResolvedValue(undefined);

    await service.handleStreamableHttpRequest(request, reply);

    expect(request.raw.headers['accept']).toBe(
  ...
  test('runStreamableHandling で例外時に 500 を返しクリーンアップする', async () => {
    ...
    await (service as any).runStreamableHandling(
      request,
      reply,
      server,
      transport,

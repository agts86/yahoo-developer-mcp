...
describe('ReverseGeocodeService', () => {
  ...
  test('lat/lng 付きでリポジトリを呼び出す', async () => {
    ...
    (mockMcpRepository.reverseGeocode as jest.Mock).mockResolvedValue(
      mockResult,
    );
  ...
  test('lat または lng が無い場合はエラー', async () => {
    const yahooAppId = 'test-app-id';
    await expect(
      service.execute({ lat: 35.0 } as any, yahooAppId),
    ).rejects.toThrow('reverseGeocode requires lat & lng');
    await expect(
      service.execute({ lng: 139.0 } as any, yahooAppId),
    ).rejects.toThrow('reverseGeocode requires lat & lng');
  });
